...
class LineChart extends ImplicitlyAnimatedWidget {
  ...
  final Function(List<double> range) onRangeSelected;
  ...
  Widget build(BuildContext context) {
    ...
    return RangeGestureDetector(
        onRangeSelected: (range) {
          ...
          final responses = range.map((item) {
            final LineTouchResponse response =
                _touchHandler?.handleTouch(FlLongPressStart(item), chartSize);
            if (response != null && response.lineBarSpots.isNotEmpty)
              return response.lineBarSpots.first.x;
            else
              return null;
          }).toList();

          widget.onRangeSelected?.call(responses);
        },
        child: CustomPaint(
          key: _chartKey,
